...
1. The recommended entry is `read`; every library language also exposes a
   ...
   returns `CookieObject[]`; Rust returns `Vec<Cookie>`. **Amended in 0.7.0:**
   `jar` is no longer warning-discarding sugar that always succeeds. Per
   [ADR 0006](0006-isolation-safe-send-selection-and-explicit-isolation-loss.md),
   it fails closed with a typed error on a snapshot holding any positively
   isolated cookie, and returns the pre-0.7.0 flattened value only when the
...
3. `header` is a view (crate-private `GetFilter`) exposed as
   ...
   identifies it. **Amended in 0.7.0:** the consuming HTTP client does not
   own send-match for a partitioned or container-scoped cookie — no format a
   client accepts (a bare URL, `http.cookiejar`, Netscape rows, a flat
   `CookieObject[]`) has a field to carry that identity through. Per
   [ADR 0006](0006-isolation-safe-send-selection-and-explicit-isolation-loss.md),
   `send_view`/`sendView`/`send-view` and `header` are the operations that
   own isolation-aware send-match, backed by one shared selection
   implementation per language; `header` now renders from `send_view`
   instead of matching independently. The client still owns ordinary
   domain/path/expiry cookie-jar bookkeeping once it has a send-safe value in
   hand.
...
- Docs lead with `read(...)` plus `send_view(...)`/`header(...)`, not
  `get(url).as_jar()`. **Amended in 0.7.0:** `jar` is documented as the
  compatibility projection — the send-safe recommendation for a snapshot
  that may hold isolated cookies is `send_view`/`header`, not `jar`.

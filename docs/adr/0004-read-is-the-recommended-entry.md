...
1. The recommended entry is `read`; every library language also exposes a
   ...
   **Amended for 0.7.0 (ADR 0006; lands with PRs 1–2):** `jar` will
   additionally fail closed with a typed error on a snapshot holding any
   positively isolated cookie. Per
   [ADR 0006](0006-isolation-safe-send-selection-and-explicit-isolation-loss.md),
   it will return today's flattened value only when the caller opts in
   explicitly or the snapshot has nothing to lose; this changes fallibility
   only — a successful `jar` call remains exactly as warning-discarding as
   it is today.
...
3. The consuming HTTP client owns send-match; Python's standard-library jar
   ...
   identifies it. **Amended for 0.7.0 (ADR 0006; lands with PRs 1–2),
   superseding "the consuming HTTP client owns send-match" above:** that
   text does not hold for a partitioned or container-scoped cookie — no
   format a client accepts (a bare URL, `http.cookiejar`, Netscape rows, a
   flat `CookieObject[]`) has a field its send-match consults for that
   identity. Per
   [ADR 0006](0006-isolation-safe-send-selection-and-explicit-isolation-loss.md),
   `send_view`/`sendView`/`send-view` and `header` will be the operations
   that own isolation-aware send-match instead, backed by one shared Rust
   selection implementation every language calls through; `header` will
   render from `send_view` instead of matching independently. The client
   still owns ordinary domain/path/expiry cookie-jar bookkeeping once it has
   a send-safe value in hand.

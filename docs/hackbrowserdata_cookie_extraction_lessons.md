...
`rookie` augments persisted Firefox cookies with one authoritative session state selected in Firefox lifecycle order: live `sessionstore-backups/recovery.jsonlz4`, clean-shutdown `sessionstore.jsonlz4` or legacy `sessionstore.js`, then stale `sessionstore-backups/previous.jsonlz4` as a last fallback. HackBrowserData reads only `cookies.sqlite`. Every existing Mozilla-family entry point that reaches `firefox_based` uses this first-valid policy rather than merging session files: `firefox`, `librewolf`, `zen`, Linux `cachy`, `firefox_profile`, direct `firefox_based`, and the Mozilla-success path of `any_browser` (plus `load`, CLI, Python, and Node calls through those APIs). New generic Mozilla report adapters associate the selected source with the correct profile and preserve duplicates until cookie identity includes container, origin, and partition attributes.

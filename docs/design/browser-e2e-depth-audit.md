...
| PR 3: active writer | Chrome and Firefox core jobs run a ready/hold/mutate/probe/close protocol against the exact database owned by a live disposable browser. Linux/macOS Chromium and all Firefox platforms exact-check complete open states and final open/closed equality. Current Windows Chromium exclusively locks a rollback-journal database, so Rust/Python/Node/CLI must return typed `source_inspection_failed` without terminating the writer; all four then exact-check the transitioned state after close. Separate Windows canaries cover elevated non-disruptive shadow-copy recovery for WAL stores and App-Bound staged-WAL recovery. Process, schema, journal, and sidecar evidence is logged. |
...
The Windows Chromium boundary above is upstream production behavior, not a CI
permission gap. Chromium enabled exclusive cookie-database locking by default
in M132 and then removed the production kill switch; only an internal testing
interface can disable it ([Chromium change 25dcb79](https://chromium.googlesource.com/chromium/src/+/25dcb79b9a5d1c470e5a1eec0b6476957c63ef84%5E%21/)).
The extractor therefore must not pretend that a rollback-journal main-file copy
is a coherent live snapshot.
...
| Chrome for Testing 151 active writer | On macOS, the exact live database was proven as `Default/Cookies` (schema 24), add/replace/delete transitions passed on all four surfaces while the browser remained alive, and the final closed snapshot equaled the final open snapshot. This also proved the harness handles both current `Default/Cookies` and older `Default/Network/Cookies` layouts. On Windows, Chrome's production exclusive locking prevents a safe read of its rollback-journal store: all four surfaces assert typed refusal while the writer stays alive, then exact-check the transitioned store after close. |

...
- **Release context:** written during the 0.6 prerelease cycle. This is an internal structure-and-domain-language refactor, not a product feature.
...
- **LANDED** as `rookie-rs/tests/report_goldens.rs` + `rookie-rs/tests/goldens/<os>/*.json`. One golden per engine holding **both** the listing and extract report, so a change that moves a source between them cannot hide. A `normalization_survives_a_different_synthetic_root` test guards the harness itself — without it every golden would still pass while pinning nothing. Regenerate with `UPDATE_GOLDENS=1 cargo test -p rookie-cookies --test report_goldens`.
  ...
  - **Deviations from this section's original scope, both deliberate:**
    1. ~~**`internet_explorer` has no golden.**~~ **CLOSED.** All three platforms are now captured via the `Capture report goldens` workflow (`.github/workflows/capture-goldens.yml`), which runs the `UPDATE_GOLDENS` capture on a real runner per OS and uploads the result — so a platform can be captured without owning a machine of that platform. Committed: `linux/{chrome,firefox}`, `macos/{chrome,firefox,safari}`, `windows/{chrome,firefox,internet_explorer}`.
       - **Capture is environment-independent, and this was verified rather than assumed.** The workflow re-captures macOS too; its output was byte-identical to the already-committed macOS goldens. Without that check there would be no basis for trusting a CI-captured Linux or Windows file.
       - **The IE golden pins the attempted-and-failed path.** A valid ESE database is too involved to synthesize, so the seed is deliberately invalid: discovery finds the candidate, populate queries it, `ese_database` acquisition is overlaid, `acquisition_attempts == 1`, and the source is `failed`. That is exactly the contract PR 0b's rewrite touches (candidate → query → overlay → commit), so the engine that previously had no report-level coverage now has the part that matters most to this program. It does **not** cover a successful ESE decode.
       - Windows also confirms the harness's JSON-escaped `\\` root replacement works: its goldens carry `<ROOT>\\AppData\\...`.
...
- **[Rev 2] Implementation notes, found while building this PR** (corrections to the spec above, not deviations from it):
  1. **PR 1 has a two-tower window *inside itself*.** The Files list and the Wrapper-draft fate table both say `EngineSourceDraft` / `EngineProfileDraft` / `EngineExtractionDraft` are deleted in PR 1, which reads as a single step. They cannot be: whichever engine migrates first leaves the others still naming the old types, so PR 1 is internally ordered — migrate Gecko, then Safari/IE, and only then delete. Both towers compile side by side in between, and one test (`unreadable_non_chromium_roots_are_detected_failures_at_public_boundaries`) grows a `browser_id == "firefox"` branch for the duration, with identical assertions on both sides.
  2. **Moving `SourceAcquisition` / `SourceFailureStage` into `source.rs` is mandatory, not optional.** Stage 5 says "re-export from `registry` for one PR if import churn needs a landing pad", which reads as a convenience. Defining them in both places produces two distinct types and `mismatched types` at the first tower boundary. `registry.rs` must delete its copies and re-export `source.rs`'s so both towers name one type.
  3. **`Source::cookies()` is load-bearing, not decorative.** Decision 13 specifies the `#[cfg(test)]` projection from `records`; characterization tests that assert on cookie contents have no other way to read a source once the `cookies` field is gone. Omitting it blocks the migration.

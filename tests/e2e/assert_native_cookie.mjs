...
if (!["safari", "internet_explorer"].includes(browser) || !existsSync(dbPath)) {
  console.error("ROOKIE_E2E_BROWSER_ID and ROOKIE_E2E_COOKIE_DB must identify Safari or IE");
  process.exit(2);
}
...
const explicit = await rookieCookies.extractFromPath(dbPath, { domains: [domain] });
const discovered =
  browser === "safari"
    ? await rookieCookies.safari([domain])
    : await rookieCookies.internetExplorer([domain]);

for (const [surface, cookies] of [
  ["extractFromPath", explicit],
  [browser, discovered],
]) {
  const seeded = cookies.find(({ name }) => name === expectedName);
  if (!seeded) {
    throw new Error(`${surface}: '${expectedName}' missing from ${cookies.length} cookies`);
  }
...
console.log(
  `rookie-cookies (${process.platform}, ${browser}): ${expectedName}=${expectedValue} verified ` +
    `(explicit=${explicit.length}, discovered=${discovered.length})`,
);

...
mod tests {
  ...
  fn unsupported_leaf_keeps_exact_legacy_errors() {
    ...
    assert_eq!(
      chromium_from_path(PathBuf::from("Cookies"), None, None)
        .unwrap_err()
        .to_string(),
      "Chromium cookie extraction is unsupported on this Unix platform"
    );
    assert_eq!(
      safari_from_path(PathBuf::from("Cookies.binarycookies"), None)
        .unwrap_err()
        .to_string(),
      "Safari binary cookie files are only supported on macOS"
    );
    assert_eq!(
      internet_explorer_from_path(PathBuf::from("WebCacheV01.dat"), None)
        .unwrap_err()
        .to_string(),

...
fn shared_cipher_loop_never_prefers_plaintext_over_ciphertext() {
  ...
  let encrypted_wins = decrypt_encrypted_value_with_cipher_adapter(
    ...
    CipherAdapter {
      ...
      decrypt_legacy: |_: &[u8]| panic!("malformed ciphertext must fail before legacy decryption"),
    },
  )
...
fn shared_cipher_loop_routes_fallible_legacy_plaintext_through_shared_decode() {
  ...
  let decoded = decrypt_encrypted_value_with_cipher_adapter(
    ...
    CipherAdapter {
      candidate_key_length: Some(4),
      validate_keyed_envelope: |_: &[u8]| panic!("legacy route must not validate a keyed envelope"),
      decrypt_candidate: |_: &[u8], _: &[u8]| panic!("legacy route must not try a keyed candidate"),
      decrypt_legacy: |_: &[u8]| {
...
fn chromium_mock_keychain_known_answer() {
  ...
  let ciphertext = [
    0x76, 0x31, 0x30, 0xbf, 0x08, 0x6d, 0x20, 0x56, 0x86, 0x1a, 0x80, 0xde, 0x82, 0x5f, 0xc9, 0x35,
    0x86, 0x86, 0x30, 0x64, 0x4f, 0x2c, 0xa1, 0x87, 0x45, 0x02, 0x13, 0xae, 0x66, 0x81, 0xb4, 0xd6,
    0x43, 0xd1, 0x9b, 0x25, 0x81, 0xc8, 0x5c, 0x88, 0x78, 0xc1, 0xbc, 0x97, 0xe7, 0x26, 0xa1, 0x0e,
    0x51, 0xea, 0x77,
  ];
  let plaintext = [
...
fn decrypt_encrypted_value_invalid_utf8_returns_error() {
  ...
  assert!(
    decrypt_encrypted_value(".example.com", "".to_string(), &encrypted_value, &[key], 23,).is_err()
  );
}

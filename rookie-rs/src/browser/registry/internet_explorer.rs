...
/// or failed query overlays the `EseDatabase` acquisition strategy after the
/// attempt, matching the frozen listing-to-extract behaviour.
      ...
      match query(&source.origin.path, domains) {
        Ok(rows) => {
          ...
          source.stats = SourceStats {
            rows_seen: rows.records_seen,
            cookies_emitted: rows.records.len(),
            rows_skipped: rows.records_skipped,
            rows_rejected: rows.records_rejected,
            provider_failures: 0,
          };
          source.records = rows.records;
          source.push_row_read_failed(rows.row_error);
        }
        Err(error) => {
...
pub(crate) fn legacy_internet_explorer_outcome_with_runtime(
  ...
  let outcome = populate_internet_explorer_sources(listing, domains.as_deref(), |path, domains| {
    query_internet_explorer_non_disruptive(path, domains, |path, domains, force_kill| {
      crate::browser::internet_explorer::internet_explorer_outcome_with_runtime(
        path, domains, force_kill, runtime,
      )
